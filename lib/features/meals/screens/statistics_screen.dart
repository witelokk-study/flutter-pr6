...
class StatisticsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final allMeals = mealsContainer.meals;

    if (allMeals.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Статистика')),
        body: const Center(
          child: Text('Недостаточно данных для отображения статистики'),
        ),
      );
    }
    ...
    final Map<String, double> caloriesByDish = {};
    for (final meal in allMeals) {
      caloriesByDish[meal.name] =
          (caloriesByDish[meal.name] ?? 0) + (meal.calories ?? 0);
    }

    final topMeals = caloriesByDish.entries.toList()
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text(
                'Всего приёмов пищи: ${allMeals.length}',
                style: const TextStyle(fontSize: 16),
              ),
              const SizedBox(height: 6),
              Text(
                'Общее количество калорий: ${caloriesByDate.values.reduce((a, b) => a + b).toStringAsFixed(0)} ккал',
                style: const TextStyle(fontSize: 16),
              ),
              const SizedBox(height: 30),
